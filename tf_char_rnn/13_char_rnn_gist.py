...
def create_model(seq, temp, vocab, hidden=HIDDEN_SIZE):
    ...
    W = tf.get_variable("W", shape=[HIDDEN_SIZE,len(vocab)], initializer=xavier_init )
    b1 = tf.get_variable("b1", shape=[len(vocab)], initializer=xavier_init )
    outputs = output[:,:-1]
    _op = tf.reshape(outputs,shape=[-1,HIDDEN_SIZE])
    labels = seq[:,1:]
    _labels = tf.reshape(labels,shape=[-1,len(vocab)])
    logits = tf.add(tf.matmul(_op,W),b1)
    #logits = tf.contrib.layers.fully_connected(output, len(vocab), None)
    loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=logits, labels=_labels))
    # sample the next word from Maxwell-Boltzmann Distribution with temperature temp
    sample = tf.multinomial(tf.exp(logits / temp), 1)[:, 0]
